# sosta chapri selfbot maked by @rafid_0009 :) 
# Give your "TOKEN" & "PREFIX" in .env file
...
Is_Auto_Hunt_Enabled = False
AutoHunt_Channel_ID = None
...
prefix = "."
client = commands.Bot(command_prefix=prefix, intents=intents, self_bot=True)  # Removed self_bot=True




async def SendMessageWithRandomDelay(message, timer=14000):
    while Is_Auto_Hunt_Enabled:
        # প্রতিবার নতুন random delay নাও
        random_time = random.randint(0, 4000) + timer

        channel_id = AutoHunt_Channel_ID
        channel = client.get_channel(channel_id)

        if channel is None or not isinstance(channel, discord.TextChannel):
            print(f"Channel with ID {channel_id} not found or is not a text channel")
        else:
            await channel.send(message)
            print(f"Sent message: {message} (next in {random_time / 1000:.2f}s)")

        # delay
        await asyncio.sleep(random_time / 1000)  # মিলিসেকেন্ড → সেকেন্ডে
...
@client.command()
async def setup(ctx, autohunt_channel_id : int):

    global AutoHunt_Channel_ID   # এই লাইন দরকার
    
    AutoHunt_Channel_ID = autohunt_channel_id

    channel = client.get_channel(autohunt_channel_id)

    await ctx.send(f"AutoHunt Channel Set To : {channel}")

@client.command()
async def autohunt(ctx, mode: str = None):
    global Is_Auto_Hunt_Enabled

    if mode is None:
        await ctx.send(f"Usage: `{prefix}autohunt on` or `{prefix}autohunt off`")
        return

    if mode.lower() == "on":
        if Is_Auto_Hunt_Enabled:
            await ctx.send("AutoHunt already running!")
            return

        Is_Auto_Hunt_Enabled = True
        await ctx.send("✅ AutoHunt started!")

        # owo task
        asyncio.create_task(SendMessageWithRandomDelay("owo hunt"))
        asyncio.create_task(SendMessageWithRandomDelay("owo battle"))
        asyncio.create_task(SendMessageWithRandomDelay("owo pray", 300000))

    elif mode.lower() == "off":
        if not Is_Auto_Hunt_Enabled:
            await ctx.send("AutoHunt already stopped!")
            return

        Is_Auto_Hunt_Enabled = False
        await ctx.send("🛑 AutoHunt stopped!")

    else:
        await ctx.send(f"Invalid mode! Use `{prefix}autohunt on` or `{prefix}autohunt off`.")
...
async def joinvc(ctx, channel_id: int):
    try:
        ...
        VcState.IsUserInVc = True
        VcState.VC_ID = channel_id
...
async def leavevc(ctx):
    try:
        # Check if the bot is connected to any voice channel in this guild
        ...
        # Reset your VC state if you’re using one
        VcState.IsUserInVc = False
        VcState.VC_ID = None

    except Exception as e:
        await ctx.send(f"⚠️ An error occurred while leaving the voice channel: {e}")
